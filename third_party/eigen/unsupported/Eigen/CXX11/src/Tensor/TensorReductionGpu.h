...
// same
// output address. It basically reads the current value of the output variable,
// and
// attempts to update it with the new value. If in the meantime another gpu
// thread
// updated the content of the output address it will try again.
template <typename T, typename R>
__device__ EIGEN_ALWAYS_INLINE void atomicReduce(T* output, T accum,
                                                 R& reducer) {
  ...
  if (sizeof(T) == 4) {
    ...
    while ((readback = atomicCAS((unsigned int*)output, oldval, newval)) !=
           oldval) {
      oldval = readback;
      newval = oldval;
      reducer.reduce(accum, reinterpret_cast<T*>(&newval));
    ...
    while ((readback = atomicCAS((unsigned long long*)output, oldval,
                                 newval)) != oldval) {
      oldval = readback;
      newval = oldval;
      reducer.reduce(accum, reinterpret_cast<T*>(&newval));
...
// reduction should be associative since reduction is not atomic in wide vector
// but atomic in half2 operations
template <typename R>
...
template <>
__device__ inline void atomicReduce(float* output, float accum,
                                    SumReducer<float>&) {
#if (defined(EIGEN_HIP_DEVICE_COMPILE) &&       \
     defined(__HIP_ARCH_HAS_WARP_SHUFFLE__)) || \
    (EIGEN_CUDA_ARCH >= 300)
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionInitKernel(
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernel(
  ...
  Index max_iter =
      numext::mini<Index>(num_coeffs - first_index, NumPerThread * BlockSize);
...
#pragma unroll
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
    ...
    // This is needed because when Type == double, hipcc will give a "call to
    // __shfl_down is ambguous" error
    // and list the float and int versions of __shfl_down as the candidate
    // functions.
    if (std::is_floating_point<typename Self::CoeffReturnType>::value) {
      reducer.reduce(__shfl_down(static_cast<float>(accum), offset, warpSize),
                     &accum);
    } else {
      reducer.reduce(__shfl_down(static_cast<int>(accum), offset, warpSize),
                     &accum);
    }
#elif defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000
    reducer.reduce(__shfl_down(accum, offset, warpSize), &accum);
#else
    reducer.reduce(__shfl_down_sync(0xFFFFFFFF, accum, offset, warpSize),
                   &accum);
#endif
  }
...
ReductionInitFullReduxKernelHalfFloat(
    Reducer reducer, const Self input, Index num_coeffs,
...
template <typename Self, typename Reducer, typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionInitKernelHalfFloat(
    Reducer reducer, const Self input, Index num_coeffs, half* output) {
  const Index thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  const Index num_threads = blockDim.x * gridDim.x;
...
#pragma unroll
  for (int offset = warpSize / 2; offset > 0; offset /= 2) {
    ...
    for (int i = 0; i < packet_width / 2; i++) {
      // FIXME : remove this workaround once we have native half/half2 support
      // for __shfl_down
      union {
        int i;
        half2 h;
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void ReductionCleanupKernelHalfFloat(
    Op reducer, half* output, packet_traits<Eigen::half>::type* scratch) {
...
struct FullReductionLauncher<
  ...
  static void run(const Self& self, Op& reducer, const GpuDevice& device,
                  OutputType* output, typename Self::Index num_coeffs) {
...
struct FullReductionLauncher<Self, Op, Eigen::half, false> {
  static void run(const Self&, Op&, const GpuDevice&, half*,
                  typename Self::Index) {
    gpu_assert(false &&
...
struct FullReductionLauncher<Self, Op, Eigen::half, true> {
  static void run(const Self& self, Op& reducer, const GpuDevice& device,
                  half* output, typename Self::Index num_coeffs) {
    typedef typename Self::Index Index;
    typedef typename packet_traits<Eigen::half>::type PacketType;
    ...
    if (num_blocks > 1) {
      // We initialize the output and the scrathpad outside the reduction kernel
      // when we can't be sure that there
      // won't be a race conditions between multiple thread blocks.
      LAUNCH_GPU_KERNEL(
          (ReductionInitFullReduxKernelHalfFloat<Self, Op, Index>), 1, 1, 0,
          device, reducer, self, num_coeffs, scratch);
    }

    LAUNCH_GPU_KERNEL((FullReductionKernelHalfFloat<block_size, num_per_thread,
                                                    Self, Op, Index>),
                      num_blocks, block_size, 0, device, reducer, self,
                      num_coeffs, output, scratch);

    if (num_blocks > 1) {
      LAUNCH_GPU_KERNEL((ReductionCleanupKernelHalfFloat<Op>), 1, 1, 0, device,
                        reducer, output, scratch);
    }
  }
};
#endif  // EIGEN_HAS_GPU_FP16
...
// of doubles, floats and half floats
  ...
  static void run(const Self& self, Op& reducer, const GpuDevice& device,
                  OutputType* output) {
    gpu_assert(HasOptimizedImplementation &&
    ...
    FullReductionLauncher<
        Self, Op, OutputType,
        reducer_traits<Op, GpuDevice>::PacketAccess>::run(self, reducer, device,
                                                          output, num_coeffs);
  }
};
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void InnerReductionKernel(
    Reducer reducer, const Self input, Index num_coeffs_to_reduce,
    Index num_preserved_coeffs, typename Self::CoeffReturnType* output) {
#if (defined(EIGEN_HIP_DEVICE_COMPILE) &&       \
     defined(__HIP_ARCH_HAS_WARP_SHUFFLE__)) || \
    (EIGEN_CUDA_ARCH >= 300)
  ...
  const Index input_col_blocks =
      divup<Index>(num_coeffs_to_reduce, blockDim.x * NumPerThread);
  const Index num_input_blocks = input_col_blocks * num_preserved_coeffs;

  const Index num_threads = blockDim.x * gridDim.x;
  const Index thread_id = blockIdx.x * blockDim.x + threadIdx.x;

  // Initialize the output values if they weren't initialized by the
  // ReductionInitKernel
  if (gridDim.x == 1) {
    for (Index i = thread_id; i < num_preserved_coeffs; i += num_threads) {
  ...
  for (Index i = blockIdx.x; i < num_input_blocks; i += gridDim.x) {
    ...
    if (row < num_preserved_coeffs) {
      ...
      const Index col_begin =
          col_block * blockDim.x * NumPerThread + threadIdx.x;
      ...
      for (Index j = 0; j < NumPerThread; j += unroll_times) {
        ...
        if (last_col >= num_coeffs_to_reduce) {
          for (Index col = col_begin + blockDim.x * j;
               col < num_coeffs_to_reduce; col += blockDim.x) {
            const Type val =
...
#pragma unroll
          for (int k = 0; k < unroll_times; ++k) {
            ...
            reducer.reduce(input.m_impl.coeff(row * num_coeffs_to_reduce + col),
                           &reduced_val);
          }
        }
      }
...
#if defined(EIGEN_HIPCC)
        // use std::is_floating_point to determine the type of reduced_val
        // This is needed because when Type == double, hipcc will give a "call
        // to __shfl_down is ambguous" error
        // and list the float and int versions of __shfl_down as the candidate
        // functions.
        if (std::is_floating_point<Type>::value) {
          reducer.reduce(__shfl_down(static_cast<float>(reduced_val), offset),
                         &reduced_val);
        } else {
          reducer.reduce(__shfl_down(static_cast<int>(reduced_val), offset),
                         &reduced_val);
        }
#elif defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000
        reducer.reduce(__shfl_down(reduced_val, offset), &reduced_val);
#else
        reducer.reduce(__shfl_down_sync(0xFFFFFFFF, reduced_val, offset),
                       &reduced_val);
#endif
      }
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void InnerReductionKernelHalfFloat(
  ...
  const Index input_col_blocks =
      divup<Index>(num_coeffs_to_reduce, blockDim.x * NumPerThread * 2);
  const Index num_input_blocks =
      divup<Index>(input_col_blocks * num_preserved_coeffs, 2);
...
#pragma unroll
      for (int offset = warpSize / 2; offset > 0; offset /= 2) {
        ...
        for (int i = 0; i < packet_width / 2; i++) {
          // FIXME : remove this workaround once we have native half/half2
          // support for __shfl_down
          union {
            int i;
            half2 h;
          } wka_in1, wka_out1;
...
struct InnerReductionLauncher {
  static EIGEN_DEVICE_FUNC bool run(const Self&, Op&, const GpuDevice&,
                                    OutputType*, typename Self::Index,
                                    typename Self::Index) {
    gpu_assert(false &&
               "Should only be called to reduce doubles, floats and half "
               "floats on a gpu device");
    return true;
  }
};
...
struct InnerReductionLauncher<
    ...
    typename internal::enable_if<
        internal::is_same<float, OutputType>::value ||
            internal::is_same<double, OutputType>::value,
        void>::type> {
  static bool run(const Self& self, Op& reducer, const GpuDevice& device,
                  OutputType* output, typename Self::Index num_coeffs_to_reduce,
    ...
    if (num_blocks > 1) {
      // We initialize the outputs outside the reduction kernel when we can't be
      // sure that there
      // won't be a race conditions between multiple thread blocks.
      ...
      const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
      LAUNCH_GPU_KERNEL((ReductionInitKernel<OutputType, Index>), num_blocks,
                        1024, 0, device, reducer.initialize(),
...
struct InnerReductionLauncher<Self, Op, Eigen::half, false> {
  static bool run(const Self&, Op&, const GpuDevice&, half*,
...
struct InnerReductionLauncher<Self, Op, Eigen::half, true> {
  static bool run(const Self& self, Op& reducer, const GpuDevice& device,
                  half* output, typename Self::Index num_coeffs_to_reduce,
                  typename Self::Index num_preserved_vals) {
    typedef typename Self::Index Index;
    ...
    if (num_blocks > 1) {
      ...
      // sure that there
      // won't be a race conditions between multiple thread blocks.
      LAUNCH_GPU_KERNEL((ReductionInitKernelHalfFloat<Self, Op, Index>), 1, 1,
                        0, device, reducer, self, num_preserved_vals, output);
    }

    LAUNCH_GPU_KERNEL(
        (InnerReductionKernelHalfFloat<num_per_thread, Self, Op, Index>),
        num_blocks, block_size, 0, device, reducer, self, num_coeffs_to_reduce,
        num_preserved_vals, output);
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void OuterReductionKernel(
  ...
  // Do the reduction.
  const Index max_iter =
      num_preserved_coeffs * divup<Index>(num_coeffs_to_reduce, NumPerThread);
  for (Index i = thread_id; i < max_iter; i += num_threads) {
    ...
    for (Index j = input_row; j < max_row; j++) {
      typename Self::CoeffReturnType val =
          input.m_impl.coeff(j * num_preserved_coeffs + input_col);
      reducer.reduce(val, &reduced_val);
    }
...
struct OuterReducer<Self, Op, GpuDevice> {
  ...
  static
#if !defined(EIGEN_HIPCC)
      // FIXME :  leaving this EIGEN_DEVICE_FUNC in, results in the following
      // runtime error
      //          (in the cxx11_tensor_reduction_gpu test)
      //
      // terminate called after throwing an instance of 'std::runtime_error'
      //   what():  No device code available for function:
      //   _ZN5Eigen8internal20OuterReductionKernelIL...
      //
      // don't know why this happens (and why is it a runtime error instead of a
      // compile time error)
      //
...
#endif
      bool
      run(const Self&, Op&, const Device&, OutputType*, typename Self::Index,
          typename Self::Index) {
    gpu_assert(
        false &&
  ...
  static bool run(const Self& self, Op& reducer, const GpuDevice& device,
                  float* output, typename Self::Index num_coeffs_to_reduce,
                  typename Self::Index num_preserved_vals) {
    typedef typename Self::Index Index;
    ...
    if (num_blocks > 1) {
      // We initialize the outputs in the reduction kernel itself when we don't
      // have to worry
      ...
      const int max_blocks = device.getNumGpuMultiProcessors() *
                             device.maxGpuThreadsPerMultiProcessor() / 1024;
      const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
      LAUNCH_GPU_KERNEL((ReductionInitKernel<float, Index>), num_blocks, 1024,
                        0, device, reducer.initialize(), num_preserved_vals,
                        output);
    }
