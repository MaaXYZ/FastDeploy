...
// clang is incompatible with the CUDA syntax wrt making a kernel a class
// friend,
...
template <typename Op, typename Dims, typename XprType,
          template <class> class MakePointer_>
struct nested<
    TensorReductionOp<Op, Dims, XprType, MakePointer_>, 1,
...
struct are_inner_most_dims<ReducedDims, NumTensorDims, ColMajor> {
  ...
  static const bool tmp3 = index_statically_eq<ReducedDims>(
      array_size<ReducedDims>::value - 1, array_size<ReducedDims>::value - 1);
  static const bool value = tmp1 & tmp2 & tmp3;
};
template <typename ReducedDims, int NumTensorDims>
struct are_inner_most_dims<ReducedDims, NumTensorDims, RowMajor> {
  static const bool tmp1 = indices_statically_known_to_increase<ReducedDims>();
  static const bool tmp2 = index_statically_eq<ReducedDims>(
      0, NumTensorDims - array_size<ReducedDims>::value);
  static const bool tmp3 = index_statically_eq<ReducedDims>(
      array_size<ReducedDims>::value - 1, NumTensorDims - 1);
  static const bool value = tmp1 & tmp2 & tmp3;
};
template <typename ReducedDims, int NumTensorDims>
...
struct preserve_inner_most_dims<ReducedDims, NumTensorDims, RowMajor> {
  static const bool tmp1 = indices_statically_known_to_increase<ReducedDims>();
  static const bool tmp2 = index_statically_lt<ReducedDims>(
      array_size<ReducedDims>::value - 1, NumTensorDims - 1);
  static const bool value = tmp1 & tmp2;
};
#endif
...
struct GenericDimReducer {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
      const Self& self, typename Self::Index firstIndex, Op& reducer,
      typename Self::CoeffReturnType* accum) {
    EIGEN_STATIC_ASSERT((DimIndex > 0), YOU_MADE_A_PROGRAMMING_MISTAKE);
    for (int j = 0; j < self.m_reducedDims[DimIndex]; ++j) {
      const typename Self::Index input =
          firstIndex + j * self.m_reducedStrides[DimIndex];
      GenericDimReducer<DimIndex - 1, Self, Op>::reduce(self, input, reducer,
                                                        accum);
    }
  }
};
...
struct GenericDimReducer<0, Self, Op> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
    ...
    for (int j = 0; j < self.m_reducedDims[0]; ++j) {
      const typename Self::Index input =
          firstIndex + j * self.m_reducedStrides[0];
...
struct GenericDimReducer<-1, Self, Op> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
      const Self& self, typename Self::Index index, Op& reducer,
      typename Self::CoeffReturnType* accum) {
...
struct InnerMostDimReducer<Self, Op, true, true> {
  ...
  reduce(const Self& self, typename Self::Index firstIndex,
    ...
    if (numValuesToReduce > packetSize * kLeafSize) {
      ...
      for (typename Self::Index j = UnrollSize; j < VectorizedSize;
           j += packetSize) {
        reducer.reducePacket(
            self.m_impl.template packet<Unaligned>(firstIndex + j), &paccum);
      }
...
struct InnerMostDimPreserver {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
      const Self&, typename Self::Index, Op&,
...
struct InnerMostDimPreserver<DimIndex, Self, Op, true> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
    ...
    for (typename Self::Index j = 0; j < self.m_reducedDims[DimIndex]; ++j) {
      const typename Self::Index input =
          firstIndex + j * self.m_reducedStrides[DimIndex];
      InnerMostDimPreserver<DimIndex - 1, Self, Op>::reduce(self, input,
                                                            reducer, accum);
    }
  }
};
...
struct InnerMostDimPreserver<0, Self, Op, true> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void reduce(
    ...
    for (typename Self::Index j = 0; j < self.m_reducedDims[0]; ++j) {
      const typename Self::Index input =
          firstIndex + j * self.m_reducedStrides[0];
      reducer.reducePacket(self.m_impl.template packet<Unaligned>(input),
                           accum);
    }
  }
};
...
struct FullReducer {
  ...
  static EIGEN_DEVICE_FUNC void run(
      const Self& self, Op& reducer, const Device&,
...
struct FullReducerShard {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void run(
      const Self& self, typename Self::Index firstIndex,
      typename Self::Index numValuesToReduce, Op& reducer,
...
struct FullReducer<Self, Op, ThreadPoolDevice, Vectorizable> {
  ...
  static void run(const Self& self, Op& reducer, const ThreadPoolDevice& device,
    ...
    MaxSizeVector<typename Self::CoeffReturnType> shards(numblocks,
                                                         reducer.initialize());
    for (Index i = 0; i < numblocks; ++i) {
      device.enqueue_with_barrier(
          &barrier, &FullReducerShard<Self, Op, Vectorizable>::run, self,
...
struct InnerReducer {
  ...
  EIGEN_DEVICE_FUNC static bool run(const Self&, Op&, const Device&,
...
ReductionInitFullReduxKernelHalfFloat(R, const S, I_,
                                      internal::packet_traits<half>::type*);
template <int B, int N, typename S, typename R, typename I_>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void FullReductionKernelHalfFloat(
    R, const S, I_, half*, internal::packet_traits<half>::type*);
template <int NPT, typename S, typename R, typename I_>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void InnerReductionKernelHalfFloat(
    R, const S, I_, I_, half*);

#endif

template <int NPT, typename S, typename R, typename I_>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void InnerReductionKernel(
    R, const S, I_, I_, typename S::CoeffReturnType*);

template <int NPT, typename S, typename R, typename I_>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void OuterReductionKernel(
    R, const S, I_, I_, typename S::CoeffReturnType*);
#endif

/**
 * For SYCL, the return type of the reduction is deduced from the initialize
 * method of the given Op.
 * This allows the reduction to have a different type for the accumulator than
 * the input data type.
 * If this is the case, the functor needs to have two reduce method: one for
 * reducing an element of the input
 * with the accumulator and the other for reducing two accumulators.
 * Such a reducer can be useful for instance when the accumulator is a boolean
 * or a bitset that checks for
 * some properties of the input.
 */
template <typename Op, typename CoeffReturnType>
struct ReductionReturnType {
#if defined(EIGEN_USE_SYCL)
  typedef typename remove_const<decltype(std::declval<Op>().initialize())>::type
      type;
#else
...
class TensorReductionOp
    : public TensorBase<TensorReductionOp<Op, Dims, XprType, MakePointer_>,
                        ReadOnlyAccessors> {
 public:
  ...
  typedef typename Eigen::internal::traits<TensorReductionOp>::StorageKind
      StorageKind;
  typedef typename Eigen::internal::traits<TensorReductionOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorReductionOp(const XprType& expr,
                                                          const Dims& dims)
...
template <typename Op, typename Dims, typename ArgType,
          template <class> class MakePointer_, typename Device>
struct TensorReductionEvaluatorBase<
    const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device> {
  ...
  typedef typename internal::conditional<NumOutputDims == 0, Sizes<>,
                                         DSizes<Index, NumOutputDims> >::type
      Dimensions;
  typedef typename XprType::Scalar Scalar;
  typedef TensorReductionEvaluatorBase<
      const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
      Self;
  static const bool InputPacketAccess =
      TensorEvaluator<ArgType, Device>::PacketAccess;
  typedef typename internal::ReductionReturnType<
      Op, typename XprType::CoeffReturnType>::type CoeffReturnType;
  ...
  static const bool ReducingInnerMostDims =
      internal::are_inner_most_dims<Dims, NumInputDims, Layout>::value;
  static const bool PreservingInnerMostDims =
      internal::preserve_inner_most_dims<Dims, NumInputDims, Layout>::value;
  ...
  TensorReductionEvaluatorBase(const XprType& op, const Device& device)
      : m_impl(op.expression(), device),
        ...
        m_result(NULL),
        m_device(device) {
    EIGEN_STATIC_ASSERT((NumInputDims >= NumReducedDims),
                        YOU_MADE_A_PROGRAMMING_MISTAKE);
    EIGEN_STATIC_ASSERT((!ReducingInnerMostDims | !PreservingInnerMostDims |
                         (NumReducedDims == NumInputDims)),
                        YOU_MADE_A_PROGRAMMING_MISTAKE);
    ...
    const typename TensorEvaluator<ArgType, Device>::Dimensions& input_dims =
        m_impl.dimensions();
    internal::DimInitializer<Dimensions>::run(input_dims, m_reduced,
                                              &m_dimensions, &m_reducedDims);
      ...
      if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
        ...
        for (int i = 1; i < NumOutputDims; ++i) {
          ...
          m_fastOutputStrides[i] =
              internal::TensorIntDivisor<Index>(m_outputStrides[i]);
        }
      } else {
  ...
  bool evalSubExprsIfNeededCommon(EvaluatorPointerType data) {
    ...
    if ((RunningFullReduction && RunningOnSycl) ||
      ...
      if (!data) {
        m_result = static_cast<EvaluatorPointerType>(m_device.get(
            (CoeffReturnType*)m_device.allocate_temp(sizeof(CoeffReturnType))));
        data = m_result;
        need_assign = true;
      }
      Op reducer(m_reducer);
      internal::FullReducer<Self, Op, Device>::run(*this, reducer, m_device,
                                                   data);
      return need_assign;
    }
    ...
    // Attempt to use an optimized reduction.
      ...
      if (internal::InnerReducer<Self, Op,
                                 Device>::HasOptimizedImplementation &&
          (reducing_inner_dims || ReducingInnerMostDims)) {
        const Index num_values_to_reduce = internal::array_prod(m_reducedDims);
        const Index num_coeffs_to_preserve = internal::array_prod(m_dimensions);
        if (!data) {
          if ((num_coeffs_to_preserve < 1024 &&
               num_values_to_reduce > num_coeffs_to_preserve &&
      ...
      if (internal::OuterReducer<Self, Op,
                                 Device>::HasOptimizedImplementation &&
      ...
      // must break into two subexpression and use the SYCL generic Reducer on
      // the device.
      if (RunningOnSycl) {
        const Index num_values_to_reduce = internal::array_prod(m_reducedDims);
        const Index num_coeffs_to_preserve = internal::array_prod(m_dimensions);
  ...
  packet(Index index) const {
    ...
    eigen_assert(index + PacketSize - 1 <
    ...
    if (ReducingInnerMostDims) {
      ...
      for (Index i = 0; i < PacketSize; ++i) {
        ...
        values[i] = internal::InnerMostDimReducer<Self, Op>::reduce(
            *this, firstIndex + i * num_values_to_reduce, num_values_to_reduce,
            reducer);
      }
    } else if (PreservingInnerMostDims) {
      const Index firstIndex = firstInput(index);
      const int innermost_dim =
          (static_cast<int>(Layout) == static_cast<int>(ColMajor))
              ? 0
              : NumOutputDims - 1;
      // TBD: extend this the the n innermost dimensions that we preserve.
      if (((firstIndex % m_dimensions[innermost_dim]) + PacketSize - 1) <
          m_dimensions[innermost_dim]) {
        Op reducer(m_reducer);
        typename Self::PacketReturnType accum =
            reducer
                .template initializePacket<typename Self::PacketReturnType>();
  ...
  costPerCoeff(bool vectorized) const {
    if (RunningFullReduction && m_result) {
      ...
      const double compute_cost =
          num_values_to_reduce * internal::functor_traits<Op>::Cost;
 ...
 private:
...
#if defined(EIGEN_HAS_GPU_FP16)
  template <typename S, typename R, typename I_>
  KERNEL_FRIEND void internal::ReductionInitFullReduxKernelHalfFloat(
      R, const S, I_, internal::packet_traits<Eigen::half>::type*);
  template <int B, int N, typename S, typename R, typename I_>
  KERNEL_FRIEND void internal::FullReductionKernelHalfFloat(
      R, const S, I_, half*, internal::packet_traits<Eigen::half>::type*);
  template <int NPT, typename S, typename R, typename I_>
  KERNEL_FRIEND void internal::InnerReductionKernelHalfFloat(R, const S, I_, I_,
                                                             half*);
#endif
  template <int NPT, typename S, typename R, typename I_>
  ...
  // SYCL need the Generic reducer for the case the recution algorithm is
  // neither inner, outer, and full reducer
...
// For full reductions
  ...
  static const bool RunningOnSycl =
      internal::is_same<typename internal::remove_all<Device>::type,
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>,
                       Device>
    : public TensorReductionEvaluatorBase<
          const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device> {
  typedef TensorReductionEvaluatorBase<
      const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>
      Base;
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(
      const typename Base::XprType& op, const Eigen::SyclDevice& device)
      : Base(op, device) {}
  // The coeff function in the base the recursive method which is not an
  // standard layout and cannot be used in the SYCL kernel
  // Therefore the coeff function should be overridden by for SYCL kernel
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename Base::CoeffReturnType coeff(
      typename Base::Index index) const {
    return *(this->data() + index);
  }
  // The packet function in the base the recursive method which is not an
  // standard layout and cannot be used in the SYCL kernel
  // Therefore the packet function should be overridden by for SYCL kernel
  template <int LoadMode>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename Base::PacketReturnType packet(
      typename Base::Index index) const {
    return internal::pload<typename Base::PacketReturnType>(this->data() +
                                                            index);
  }
};
