...
class IndexMapper {
 public:
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuInputPlaneToTensorInputOffset(Index p) const {
    Index inputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuOutputPlaneToTensorOutputOffset(Index p) const {
    Index outputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int d = NumDims - 1; d > NumKernelDims; --d) {
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuInputKernelToTensorInputOffset(Index i) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuOutputKernelToTensorOutputOffset(Index i) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuInputKernelToTensorInputOffset(Index i, Index j) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuOutputKernelToTensorOutputOffset(Index i, Index j) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuInputKernelToTensorInputOffset(Index i, Index j, Index k) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
    return i * m_inputStrides[offset] + j * m_inputStrides[offset + 1] +
  ...
  EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index
  mapGpuOutputKernelToTensorOutputOffset(Index i, Index j, Index k) const {
    const size_t offset = static_cast<int>(Layout) == static_cast<int>(ColMajor)
                              ? 0
                              : NumDims - NumKernelDims;
    return i * m_outputStrides[offset] + j * m_outputStrides[offset + 1] +
...
struct traits<TensorConvolutionOp<Dimensions, InputXprType, KernelXprType> > {
  ...
  // are different.
  typedef
      typename promote_storage_type<typename InputXprType::Scalar,
                                    typename KernelXprType::Scalar>::ret Scalar;
  typedef typename promote_storage_type<
      typename traits<InputXprType>::StorageKind,
      typename traits<KernelXprType>::StorageKind>::ret StorageKind;
  typedef
      typename promote_index_type<typename traits<InputXprType>::Index,
                                  typename traits<KernelXprType>::Index>::type
          Index;
  ...
  typedef typename conditional<
      Pointer_type_promotion<typename InputXprType::Scalar, Scalar>::val,
...
template <typename Dimensions, typename InputXprType, typename KernelXprType>
struct eval<TensorConvolutionOp<Dimensions, InputXprType, KernelXprType>,
            Eigen::Dense> {
  typedef const TensorConvolutionOp<Dimensions, InputXprType, KernelXprType>&
      type;
};

template <typename Dimensions, typename InputXprType, typename KernelXprType>
struct nested<TensorConvolutionOp<Dimensions, InputXprType, KernelXprType>, 1,
              typename eval<TensorConvolutionOp<Dimensions, InputXprType,
                                                KernelXprType> >::type> {
...
class TensorConvolutionOp
    : public TensorBase<
          TensorConvolutionOp<Indices, InputXprType, KernelXprType>,
          ReadOnlyAccessors> {
 public:
  ...
  typedef typename Eigen::internal::traits<TensorConvolutionOp>::StorageKind
      StorageKind;
  typedef typename Eigen::internal::traits<TensorConvolutionOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  TensorConvolutionOp(const InputXprType& input, const KernelXprType& kernel,
                      const Indices& dims)
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const typename internal::remove_all<
      typename InputXprType::Nested>::type&
  inputExpression() const {
    return m_input_xpr;
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const typename internal::remove_all<
      typename KernelXprType::Nested>::type&
...
struct TensorEvaluator<
    const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, Device> {
  typedef TensorConvolutionOp<Indices, InputArgType, KernelArgType> XprType;

  static const int NumDims = internal::array_size<
      typename TensorEvaluator<InputArgType, Device>::Dimensions>::value;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
      ...
      : m_inputImpl(op.inputExpression(), device),
        ...
        m_device(device) {
    EIGEN_STATIC_ASSERT(
        (static_cast<int>(TensorEvaluator<InputArgType, Device>::Layout) ==
         static_cast<int>(TensorEvaluator<KernelArgType, Device>::Layout)),
        YOU_MADE_A_PROGRAMMING_MISTAKE);

    const typename TensorEvaluator<InputArgType, Device>::Dimensions&
        input_dims = m_inputImpl.dimensions();
    const typename TensorEvaluator<KernelArgType, Device>::Dimensions&
        kernel_dims = m_kernelImpl.dimensions();
 ...
 private:
  ...
  EIGEN_DEVICE_FUNC void convolve(Index firstIndex, Index firstKernel,
                                  int DimIndex, CoeffReturnType& accum) const {
  ...
  EIGEN_DEVICE_FUNC void convolvePacket(Index firstIndex, Index firstKernel,
                                        int DimIndex, Packet& accum) const {
    for (int j = 0; j < m_kernelImpl.dimensions()[DimIndex]; ++j) {
      const Index input = firstIndex + j * m_indexStride[DimIndex];
      const Index kernel = firstKernel + j * m_kernelStride[DimIndex];
      if (DimIndex > 0) {
        ...
        accum = internal::pmadd<Packet>(
            m_inputImpl.template packet<Unaligned>(input),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void preloadKernel() {
    ...
    if (in_place) {
      ...
      const bool Vectorize =
          internal::IsVectorizable<Device, KernelArgType>::value;
      internal::TensorExecutor<const EvalTo, Device, Vectorize>::run(evalToTmp,
                                                                     m_device);
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void EigenConvolutionKernel1D(
  ...
  const int num_x_input =
      last_x - first_x + GetKernelSize<StaticKernelSize>()(kernelSize);
  ...
  for (int p = first_plane + threadIdx.y; p < numPlanes; p += plane_stride) {
    ...
    const int plane_input_offset =
        indexMapper.mapGpuInputPlaneToTensorInputOffset(p);
    const int plane_kernel_offset = threadIdx.y * num_x_input;
#pragma unroll
    for (int i = threadIdx.x; i < num_x_input; i += blockDim.x) {
      const int tensor_index =
          plane_input_offset +
          indexMapper.mapGpuInputKernelToTensorInputOffset(i + first_x);
...
#pragma unroll
      ...
      const int tensor_index =
          plane_output_offset +
          indexMapper.mapGpuOutputKernelToTensorOutputOffset(i + first_x);
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void EigenConvolutionKernel2D(
  ...
  const int last_x = (first_x + maxX < numX ? first_x + maxX : numX) - 1;
  const int num_x_input =
      last_x - first_x + GetKernelSize<StaticKernelSizeX>()(kernelSizeX);
  const int num_x_output = last_x - first_x + 1;

  const int first_y = blockIdx.y * maxY;
  const int last_y = (first_y + maxY < numY ? first_y + maxY : numY) - 1;
  const int num_y_input =
      last_y - first_y + GetKernelSize<StaticKernelSizeY>()(kernelSizeY);
...
#pragma unroll
      for (int i = threadIdx.x; i < num_x_output; i += blockDim.x) {
        float result = 0.0f;
#pragma unroll
        for (int l = 0; l < GetKernelSize<StaticKernelSizeY>()(kernelSizeY);
             ++l) {
          const int kernel_offset = kernelSizeX * l;
          const int input_offset =
              i + num_x_input * (j + l + plane_kernel_offset);
#pragma unroll
          for (int k = 0; k < GetKernelSize<StaticKernelSizeX>()(kernelSizeX);
               ++k) {
    ...
    for (int k = threadIdx.z; k < num_z_input; k += blockDim.z) {
      for (int j = threadIdx.y; j < num_y_input; j += blockDim.y) {
        for (int i = threadIdx.x; i < num_x_input; i += blockDim.x) {
          ...
          s[i + num_x_input * (j + num_y_input * (k + plane_kernel_offset))] =
    ...
    for (int k = threadIdx.z; k < num_z_output; k += blockDim.z) {
      for (int j = threadIdx.y; j < num_y_output; j += blockDim.y) {
        for (int i = threadIdx.x; i < num_x_output; i += blockDim.x) {
            ...
            for (int m = 0; m < kernelSizeY; ++m) {
              for (int l = 0; l < kernelSizeX; ++l) {
                result += s[i + l +
                            num_x_input *
                                (j + m +
                                 num_y_input * (k + n + plane_kernel_offset))] *
          ...
          const int tensor_index =
              plane_output_offset +
              indexMapper.mapGpuOutputKernelToTensorOutputOffset(
                  i + first_x, j + first_y, k + first_z);
          buffer[tensor_index] = result;
        }
      }
...
template <typename Indices, typename InputArgType, typename KernelArgType>
struct TensorEvaluator<
    const TensorConvolutionOp<Indices, InputArgType, KernelArgType>,
    GpuDevice> {
  typedef TensorConvolutionOp<Indices, InputArgType, KernelArgType> XprType;

  static const int NumDims = internal::array_size<
      typename TensorEvaluator<InputArgType, GpuDevice>::Dimensions>::value;
  static const int NumKernelDims = internal::array_size<Indices>::value;
  typedef typename XprType::Index Index;
  typedef DSizes<Index, NumDims> Dimensions;
  typedef typename TensorEvaluator<KernelArgType, GpuDevice>::Dimensions
      KernelDimensions;
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType& op, const GpuDevice& device)
    ...
    const typename TensorEvaluator<InputArgType, GpuDevice>::Dimensions&
        input_dims = m_inputImpl.dimensions();
    const typename TensorEvaluator<KernelArgType, GpuDevice>::Dimensions&
        kernel_dims = m_kernelImpl.dimensions();
  ...
  typedef typename XprType::CoeffReturnType CoeffReturnType;
  typedef
      typename PacketType<CoeffReturnType, GpuDevice>::type PacketReturnType;
  ...
  EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(Scalar* data) {
    ...
    if (data) {
      ...
      m_buf =
          (Scalar*)m_device.allocate(dimensions().TotalSize() * sizeof(Scalar));
      executeEval(m_buf);
      return true;
    }
  }
  ...
  EIGEN_STRONG_INLINE void preloadKernel() {
    ...
    if (in_place) {
      ...
      typedef TensorEvalToOp<const KernelArgType> EvalTo;
      EvalTo evalToTmp(local, m_kernelArg);
      const bool PacketAccess =
          internal::IsVectorizable<GpuDevice, KernelArgType>::value;
      internal::TensorExecutor<const EvalTo, GpuDevice, PacketAccess>::run(
          evalToTmp, m_device);
    ...
    switch (NumKernelDims) {
      case 1: {
        ...
        if (m_indices[0] == single_stride_dim) {
          // Maximum the reuse
          const int inner_dim =
              ((maxSharedMem / (sizeof(Scalar)) - kernel_size + 1 + 31) / 32) *
              32;
          maxX = numext::mini<int>(inner_dim, numX);
          const int maxP = numext::mini<int>(
              maxSharedMem / ((kernel_size - 1 + maxX) * sizeof(Scalar)), numP);
          block_size.x = numext::mini(maxThreadsPerBlock, maxX);
          block_size.y =
              numext::mini<int>(maxThreadsPerBlock / block_size.x, maxP);
        } else {
          // Read as much as possible alongside the inner most dimension, that
          // is the plane
          const int inner_dim =
              maxSharedMem / ((warpSize + kernel_size) * sizeof(Scalar));
          const int maxP = numext::mini<int>(inner_dim, numP);
          maxX = numext::mini<int>(
              maxSharedMem / (inner_dim * sizeof(Scalar)) - kernel_size + 1,
              numX);
        ...
        const int shared_mem =
            block_size.y * (maxX + kernel_size - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);

        const int num_x_blocks = ceil(numX, maxX);
        const int blocksPerProcessor =
        ...
        dim3 num_blocks(
            num_x_blocks,
            numext::mini<int>(num_y_blocks, ceil(numP, block_size.y)));

        // cout << "launching 1D kernel with block_size.x: " << block_size.x <<
        // " block_size.y: " << block_size.y << " num_blocks.x: " <<
        // num_blocks.x << " num_blocks.y: " << num_blocks.y << " maxX: " <<
        // maxX << " shared_mem: " << shared_mem << " in stream " <<
        // m_device.stream() << endl;
        ...
        switch (kernel_size) {
          case 4: {
            LAUNCH_GPU_KERNEL((EigenConvolutionKernel1D<
                                  TensorEvaluator<InputArgType, GpuDevice>,
                                  Index, InputDims, 4>),
                              num_blocks, block_size, shared_mem, m_device,
                              m_inputImpl, indexMapper, m_kernel, numP, numX,
                              maxX, 4, data);
            break;
          }
          case 7: {
          ...
          default: {
            LAUNCH_GPU_KERNEL((EigenConvolutionKernel1D<
                                  TensorEvaluator<InputArgType, GpuDevice>,
                                  Index, InputDims, Dynamic>),
                              num_blocks, block_size, shared_mem, m_device,
                              m_inputImpl, indexMapper, m_kernel, numP, numX,
                              maxX, kernel_size, data);
          }
        }
        break;
      }
      ...
      case 2: {
        ...
        const float scaling_factor =
            sqrtf(static_cast<float>(maxSharedMem) /
                  (sizeof(Scalar) * kernel_size_y * kernel_size_x));

        // Snap maxX to warp size
        int inner_dim = ((static_cast<int>(scaling_factor * kernel_size_x) -
                          kernel_size_x + 1 + 32) /
                         32) *
                        32;
        const int maxX = numext::mini<int>(inner_dim, numX);
        const int maxY = numext::mini<int>(
            maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1)) -
                kernel_size_y + 1,
            numY);
        const int maxP = numext::mini<int>(
            maxSharedMem / ((kernel_size_x - 1 + maxX) *
                            (kernel_size_y - 1 + maxY) * sizeof(Scalar)),
            numP);
        ...
        block_size.z =
            numext::mini<int>(1024 / (block_size.x * block_size.y), maxP);

        const int shared_mem = block_size.z * (maxX + kernel_size_x - 1) *
                               (maxY + kernel_size_y - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);
        ...
        const int blocksPerProcessor =
            numext::mini(maxBlocksPerProcessor, maxSharedMem / shared_mem);
        const int num_z_blocks = ceil(numMultiProcessors * blocksPerProcessor,
                                      num_x_blocks * num_y_blocks);

        dim3 num_blocks(
            num_x_blocks, num_y_blocks,
            numext::mini<int>(num_z_blocks, ceil(numP, block_size.z)));

        // cout << "launching 2D kernel with block_size.x: " << block_size.x <<
        // " block_size.y: " << block_size.y  << " block_size.z: " <<
        // block_size.z << " num_blocks.x: " << num_blocks.x << " num_blocks.y:
        // " << num_blocks.y << " num_blocks.z: " << num_blocks.z << " maxX: "
        // << maxX << " maxY: " << maxY << " maxP: " << maxP << " shared_mem: "
        ...
        switch (kernel_size_x) {
          case 4: {
            switch (kernel_size_y) {
              case 7: {
                LAUNCH_GPU_KERNEL((EigenConvolutionKernel2D<
                                      TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, 4, 7>),
                                  num_blocks, block_size, shared_mem, m_device,
                                  m_inputImpl, indexMapper, m_kernel, numP,
                                  numX, maxX, numY, maxY, 4, 7, data);
                break;
              }
              default: {
                LAUNCH_GPU_KERNEL((EigenConvolutionKernel2D<
                                      TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, 4, Dynamic>),
                                  num_blocks, block_size, shared_mem, m_device,
                                  m_inputImpl, indexMapper, m_kernel, numP,
                                  numX, maxX, numY, maxY, 4, kernel_size_y,
                                  data);
                break;
              }
            }
            ...
            switch (kernel_size_y) {
              case 4: {
                LAUNCH_GPU_KERNEL((EigenConvolutionKernel2D<
                                  ...
                                  num_blocks, block_size, shared_mem, m_device,
                                  m_inputImpl, indexMapper, m_kernel, numP,
                                  numX, maxX, numY, maxY, 7, 4, data);
                break;
              }
              default: {
                LAUNCH_GPU_KERNEL((EigenConvolutionKernel2D<
                                      TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, 7, Dynamic>),
                                  num_blocks, block_size, shared_mem, m_device,
                                  m_inputImpl, indexMapper, m_kernel, numP,
                                  numX, maxX, numY, maxY, 7, kernel_size_y,
                                  data);
                break;
              }
            }
          ...
          default: {
            LAUNCH_GPU_KERNEL((EigenConvolutionKernel2D<
                                  TensorEvaluator<InputArgType, GpuDevice>,
                                  Index, InputDims, Dynamic, Dynamic>),
                              num_blocks, block_size, shared_mem, m_device,
                              m_inputImpl, indexMapper, m_kernel, numP, numX,
                              maxX, numY, maxY, kernel_size_x, kernel_size_y,
                              data);
            break;
          }
        }
      ...
      case 3: {
        ...
        const int maxX = numext::mini<int>(
            128,
            numext::mini<int>(maxSharedMem / (sizeof(Scalar) * kernel_size_y *
                                              kernel_size_z) -
                                  kernel_size_x + 1,
                              numX));
        const int maxY = numext::mini<int>(
            128, numext::mini<int>(maxSharedMem / (sizeof(Scalar) *
                                                   (maxX + kernel_size_x - 1) *
                                                   kernel_size_z) -
                                       kernel_size_y + 1,
                                   numY));
        ...
        block_size.z =
            numext::mini<int>(1024 / (block_size.x * block_size.y), maxZ);
        dim3 num_blocks(ceil(numX, maxX), ceil(numY, maxY), ceil(numZ, maxZ));

        const int shared_mem = (maxX + kernel_size_x - 1) *
                               (maxY + kernel_size_y - 1) *
                               (maxZ + kernel_size_z - 1) * sizeof(Scalar);
        gpu_assert(shared_mem <= maxSharedMem);

        // cout << "launching 3D kernel with block_size.x: " << block_size.x <<
        // " block_size.y: " << block_size.y  << " block_size.z: " <<
        // block_size.z << " num_blocks.x: " << num_blocks.x << " num_blocks.y:
        // " << num_blocks.y << " num_blocks.z: " << num_blocks.z  << "
        // shared_mem: " << shared_mem << " in stream " << m_device.stream() <<
        // endl;
        ...
        LAUNCH_GPU_KERNEL(
            (EigenConvolutionKernel3D<TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims>),
            num_blocks, block_size, shared_mem, m_device, m_inputImpl,
            indexMapper, m_kernel, numP, numX, maxX, numY, maxY, numZ, maxZ,
            kernel_size_x, kernel_size_y, kernel_size_z, data);
        break;
      }
