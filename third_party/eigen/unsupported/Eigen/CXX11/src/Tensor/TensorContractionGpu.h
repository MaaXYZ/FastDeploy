...
__device__ EIGEN_STRONG_INLINE void EigenContractionKernelInternal(
    const LhsMapper lhs, const RhsMapper rhs, const OutputMapper output,
    Scalar* lhs_shmem, Scalar* rhs_shmem, const Index m_size,
  ...
  // bank conflicts when reading the LHS. The core idea is that since the
  // contracting
  // index is shared by both sides, then the contracting index should be in
  // threadIdx.x.

  // On the LHS, we pad each row inside of each block with an extra element.
  // This makes
  // each block 8 rows of 9 elements, which is 72 elements. This gives no bank
  // conflicts
  // on writes and very few 2-way conflicts on reads. There is an 8x8 grid of
  // these blocks.

  // On the RHS we just add 8 padding elements to the end of each block. This
  // gives no bank
  // conflicts on writes and also none on reads.

  // storage indices
  const Index lhs_store_idx_base =
      threadIdx.y * 72 + threadIdx.x * 9 + threadIdx.z;
  const Index rhs_store_idx_base =
      threadIdx.y * 72 + threadIdx.z * 8 + threadIdx.x;
...
// the sum across all big k blocks of the product of little k block of index (x,
// y)
// with block of index (y, z). To compute the final output, we need to reduce
// the 8 threads over y by summation.
#if defined(EIGEN_HIPCC) || \
    (defined(EIGEN_CUDA_SDK_VER) && EIGEN_CUDA_SDK_VER < 90000)
#define shuffleInc(i, j, mask) res(i, j) += __shfl_xor(res(i, j), mask)
#else
#define shuffleInc(i, j, mask) \
  res(i, j) += __shfl_xor_sync(0xFFFFFFFF, res(i, j), mask)
#endif
...
#undef writeRow
  ...
  const int max_i_write =
      numext::mini((int)((m_size - base_m - threadIdx.y + 7) / 8), 8);
  const int max_j_write =
      numext::mini((int)((n_size - base_n - threadIdx.z + 7) / 8), 8);

  if (threadIdx.x < max_i_write) {
    if (max_j_write == 8) {
      // TODO: can i trade bank conflicts for coalesced writes?
      Scalar val0 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 0];
      Scalar val1 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 1];
      Scalar val2 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 2];
      Scalar val3 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 3];
      Scalar val4 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 4];
      Scalar val5 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 5];
      Scalar val6 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 6];
      Scalar val7 =
          lhs_shmem[threadIdx.x + 8 * threadIdx.y + 64 * threadIdx.z + 512 * 7];

      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 0) = val0;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 1) = val1;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 2) = val2;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 3) = val3;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 4) = val4;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 5) = val5;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 6) = val6;
      output(base_m + threadIdx.y + 8 * threadIdx.x,
             base_n + threadIdx.z + 8 * 7) = val7;
    } else {
#pragma unroll 7
      for (int j = 0; j < max_j_write; j++) {
        Scalar val = lhs_shmem[threadIdx.x + 8 * threadIdx.y +
                               64 * threadIdx.z + 512 * j];
        output(base_m + threadIdx.y + 8 * threadIdx.x,
               base_n + threadIdx.z + 8 * j) = val;
      }
    }
  }
#undef res
...
__global__ void
    ...
    __launch_bounds__(512)
        ...
        EigenContractionKernel(const LhsMapper lhs, const RhsMapper rhs,
                               const OutputMapper output, const Index m_size,
                               const Index n_size, const Index k_size) {
  ...
  if (base_m + 63 < m_size && base_n + 63 < n_size) {
    EigenContractionKernelInternal<Scalar, Index, LhsMapper, RhsMapper,
                                   OutputMapper, false>(
...
template <typename Index, typename LhsMapper, typename RhsMapper,
          typename OutputMapper, bool CHECK_LHS_BOUNDARY,
          bool CHECK_RHS_BOUNDARY>
__device__ __forceinline__ void EigenFloatContractionKernelInternal16x16(
    const LhsMapper lhs, const RhsMapper rhs, const OutputMapper output,
    float2 lhs_shmem2[][16], float2 rhs_shmem2[][8], const Index m_size,
    const Index n_size, const Index k_size, const Index base_m,
    const Index base_n) {
  // prefetch registers
  ...
  for (Index k = 0; k < k_size; k += 16) {
    ...
    if (!CHECK_RHS_BOUNDARY) {
      if ((rhs_vert + 3) < k_size) {
        ...
        rhs_pf0 =
            rhs.template loadPacket<float4, Unaligned>(rhs_vert, rhs_horiz0);
    ...
    // ...
    rhs_shmem2[(threadIdx.x >> 3) + threadIdx.y * 2][threadIdx.x % 8] =
        make_float2(rhs_pf0.x, rhs_pf0.y);
    rhs_shmem2[(threadIdx.x >> 3) + threadIdx.y * 2 + 32][threadIdx.x % 8] =
        make_float2(rhs_pf0.z, rhs_pf0.w);

    // Row 0 (time 0) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..
    // (60, 61)
    // Row 1 (time 1) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..
    // (60, 61)
    // ...
    // Row 15 (time 15) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..
    // (60, 61)
    // Row 16 (time 0) -> features (2, 3), (6, 7), .. (30, 31), (34, 35), ..
    // (62, 63)
    // ...

    lhs_shmem2[threadIdx.y][threadIdx.x] = make_float2(lhs_pf0.x, lhs_pf0.y);
    lhs_shmem2[threadIdx.y + 16][threadIdx.x] =
        make_float2(lhs_pf0.z, lhs_pf0.w);
...
#pragma unroll
    for (int koff = 0; koff < 16; koff++) {
      ...
      float2 fr1 = rhs_shmem2[(start_feature >> 1) + 32 * ((koff % 4) / 2)]
                             [koff / 4 + (koff % 2) * 4];
      float2 fr2 = rhs_shmem2[(start_feature >> 1) + 1 + 32 * ((koff % 4) / 2)]
                             [koff / 4 + (koff % 2) * 4];
...
template <typename Index, typename LhsMapper, typename RhsMapper,
          typename OutputMapper, bool CHECK_LHS_BOUNDARY,
          bool CHECK_RHS_BOUNDARY>
__device__ __forceinline__ void EigenFloatContractionKernelInternal(
    const LhsMapper lhs, const RhsMapper rhs, const OutputMapper output,
    float2 lhs_shmem2[][32], float2 rhs_shmem2[][8], const Index m_size,
    const Index n_size, const Index k_size, const Index base_m,
    const Index base_n) {
  // prefetch registers
  ...
  for (Index k = 0; k < k_size; k += 32) {
    ...
    if (!CHECK_LHS_BOUNDARY) {
      if ((threadIdx.y / 4 + k + 24) < k_size) {
        lhs_pf0 = lhs.template loadPacket<float4, Unaligned>(
            lhs_vert, (threadIdx.y / 4 + k));
        lhs_pf1 = lhs.template loadPacket<float4, Unaligned>(
            lhs_vert, (threadIdx.y / 4 + k + 8));
        lhs_pf2 = lhs.template loadPacket<float4, Unaligned>(
            lhs_vert, (threadIdx.y / 4 + k + 16));
        lhs_pf3 = lhs.template loadPacket<float4, Unaligned>(
            lhs_vert, (threadIdx.y / 4 + k + 24));
    ...
    if (!CHECK_RHS_BOUNDARY) {
      if ((rhs_vert + 3) < k_size) {
        ...
        rhs_pf0 =
            rhs.template loadPacket<float4, Unaligned>(rhs_vert, rhs_horiz0);
        rhs_pf1 =
            rhs.template loadPacket<float4, Unaligned>(rhs_vert, rhs_horiz1);
      } else if (rhs_vert + 2 < k_size) {
        // just CHECK_RHS_BOUNDARY
        rhs_pf0.x = rhs(rhs_vert, rhs_horiz0);
        rhs_pf0.y = rhs(rhs_vert + 1, rhs_horiz0);
    ...
    // ..
    rhs_shmem2[threadIdx.y + 32][threadIdx.x] =
        make_float2(rhs_pf0.y, rhs_pf1.y);
    // Row 64 -> times (2, 6, 10, .. 30) for features 0, 1.
    // Row 65 -> times (2, 6, 10, .. 30) for features 2, 3.
    rhs_shmem2[threadIdx.y + 64][threadIdx.x] =
        make_float2(rhs_pf0.z, rhs_pf1.z);
    // Row 96 -> times (3, 7, 11, .. 31) for features 0, 1.
    // Row 97 -> times (3, 7, 11, .. 31) for features 2, 3.
    rhs_shmem2[threadIdx.y + 96][threadIdx.x] =
        make_float2(rhs_pf0.w, rhs_pf1.w);

// LHS.
// Row 0 (time 0) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..  (60,
// 61) .. (124, 125)
// Row 1 (time 1) -> features (0, 1), (4, 5), .. (28, 29), (32, 33), ..  (60,
// 61) .. (124, 125)
// ...
// Row 8 (time 0) -> features (2, 3), (6, 7), .. (30, 31), (34, 35), ..  (62,
// 63) .. (126, 127)
// Row 15 (time 7) -> features (2, 3), (6, 7), .. (30, 31), (34, 35), ..  (62,
// 63) .. (126, 127)
...
#define add_vals(a_feat1, a_feat2, f1, f2, f3, f4) \
  ...
  results[7].w += a_feat2.y * f4.y;
    ...
    lhs_shmem2[threadIdx.y / 4][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf0.x, lhs_pf0.y);
    lhs_shmem2[threadIdx.y / 4 + 8][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf1.x, lhs_pf1.y);
    lhs_shmem2[threadIdx.y / 4 + 16][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf2.x, lhs_pf2.y);
    lhs_shmem2[threadIdx.y / 4 + 24][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf3.x, lhs_pf3.y);

    lhs_shmem2[threadIdx.y / 4 + 32][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf0.z, lhs_pf0.w);
    lhs_shmem2[threadIdx.y / 4 + 40][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf1.z, lhs_pf1.w);
    lhs_shmem2[threadIdx.y / 4 + 48][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf2.z, lhs_pf2.w);
    lhs_shmem2[threadIdx.y / 4 + 56][threadIdx.x + (threadIdx.y % 4) * 8] =
        make_float2(lhs_pf3.z, lhs_pf3.w);
...
#pragma unroll
    for (int koff = 0; koff < 32; koff++) {
      ...
      float2 br2 =
          rhs_shmem2[start_feature / 2 + 1 + (koff % 4) * 32][koff / 4];
      float2 br3 =
          rhs_shmem2[start_feature / 2 + 2 + (koff % 4) * 32][koff / 4];
      float2 br4 =
          rhs_shmem2[start_feature / 2 + 3 + (koff % 4) * 32][koff / 4];
...
__global__ void
    ...
    __launch_bounds__(256)
        ...
        EigenFloatContractionKernel(const LhsMapper lhs, const RhsMapper rhs,
                                    ...
                                    const Index m_size, const Index n_size,
                                    const Index k_size) {
  ...
  if (!check_rhs) {
    if (!check_lhs128) {
      // >= 128 rows left
      EigenFloatContractionKernelInternal<Index, LhsMapper, RhsMapper,
                                          OutputMapper, false, false>(
          lhs, rhs, output, *((LHS_MEM*)lhs_shmem), *((RHS_MEM*)rhs_shmem),
          m_size, n_size, k_size, base_m, base_n);
    } else {
      EigenFloatContractionKernelInternal<Index, LhsMapper, RhsMapper,
                                          OutputMapper, true, false>(
          lhs, rhs, output, *((LHS_MEM*)lhs_shmem), *((RHS_MEM*)rhs_shmem),
          m_size, n_size, k_size, base_m, base_n);
    }
  } else {
    if (!check_lhs128) {
      // >= 128 rows left
      EigenFloatContractionKernelInternal<Index, LhsMapper, RhsMapper,
                                          OutputMapper, false, true>(
          lhs, rhs, output, *((LHS_MEM*)lhs_shmem), *((RHS_MEM*)rhs_shmem),
          m_size, n_size, k_size, base_m, base_n);
    } else {
      EigenFloatContractionKernelInternal<Index, LhsMapper, RhsMapper,
                                          OutputMapper, true, true>(
          lhs, rhs, output, *((LHS_MEM*)lhs_shmem), *((RHS_MEM*)rhs_shmem),
          m_size, n_size, k_size, base_m, base_n);
    }
  }
...
__global__ void
    ...
    __launch_bounds__(256)
#endif
        EigenFloatContractionKernel16x16(const LhsMapper lhs,
                                         const RhsMapper rhs,
                                         const OutputMapper output,
                                         const Index m_size, const Index n_size,
                                         const Index k_size) {
  ...
  if (base_m + 63 < m_size) {
    if (base_n + 63 < n_size) {
      EigenFloatContractionKernelInternal16x16<Index, LhsMapper, RhsMapper,
                                               OutputMapper, false, false>(
          lhs, rhs, output, lhs_shmem, rhs_shmem, m_size, n_size, k_size,
          base_m, base_n);
    } else {
...
struct TensorEvaluator<
  ...
  typedef TensorContractionOp<Indices, LeftArgType, RightArgType,
                              OutputKernelType>
      XprType;
  typedef
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType& op, const Device& device)
      : Base(op, device) {
    EIGEN_STATIC_ASSERT(
        (internal::is_same<OutputKernelType, const NoOpOutputKernel>::value),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(
    ...
    if (data) {
      ...
      this->m_result = static_cast<Scalar*>(this->m_device.allocate(
          this->dimensions().TotalSize() * sizeof(Scalar)));
  ...
  struct LaunchKernels {
    static void Run(const LhsMapper& lhs, const RhsMapper& rhs,
      ...
      LAUNCH_GPU_KERNEL((EigenContractionKernel<Scalar, Index, LhsMapper,
                                                RhsMapper, OutputMapper>),
                        num_blocks, block_size, 0, device, lhs, rhs, output, m,
                        n, k);
    }
  };
  ...
  template <typename Index, typename LhsMapper, typename RhsMapper,
            typename OutputMapper>
  struct LaunchKernels<float, float, Index, LhsMapper, RhsMapper,
                       OutputMapper> {
    static void Run(const LhsMapper& lhs, const RhsMapper& rhs,
                    const OutputMapper& output, Index m, Index n, Index k,
                    const GpuDevice& device) {
      if (m < 768 || n < 768) {
        ...
        LAUNCH_GPU_KERNEL(
            (EigenFloatContractionKernel16x16<Index, LhsMapper, RhsMapper,
                                              OutputMapper>),
            num_blocks, block_size, 0, device, lhs, rhs, output, m, n, k);
      } else {
        ...
        LAUNCH_GPU_KERNEL(
            (EigenFloatContractionKernel<Index, LhsMapper, RhsMapper,
                                         OutputMapper>),
            num_blocks, block_size, 0, device, lhs, rhs, output, m, n, k);
      }
    }
  ...
  void evalTyped(Scalar* buffer) const {
    ...
    LaunchKernels<LhsScalar, RhsScalar, Index, LhsMapper, RhsMapper,
                  OutputMapper>::Run(lhs, rhs, output, m, n, k, this->m_device);
  }
};
