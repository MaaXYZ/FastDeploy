...
class TensorReshapingOp
    : public TensorBase<TensorReshapingOp<NewDimensions, XprType>,
                        WriteAccessors> {
 public:
  typedef TensorBase<TensorReshapingOp<NewDimensions, XprType>, WriteAccessors>
      Base;
  ...
  typedef typename Eigen::internal::traits<TensorReshapingOp>::StorageKind
      StorageKind;
...
struct TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>,
                       Device> {
  ...
  typedef StorageMemory<typename internal::remove_const<CoeffReturnType>::type,
                        Device>
      ConstCastStorage;
...
struct TensorEvaluator<TensorReshapingOp<NewDimensions, ArgType>, Device>
    : public TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>,
                             Device>

{
  typedef TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>,
                          Device>
      Base;
  typedef TensorReshapingOp<NewDimensions, ArgType> XprType;
  typedef NewDimensions Dimensions;
...
struct eval<TensorSlicingOp<StartIndices, Sizes, XprType>, Eigen::Dense> {
  typedef const TensorSlicingOp<StartIndices, Sizes, XprType> EIGEN_DEVICE_REF
      type;
};

template <typename StartIndices, typename Sizes, typename XprType>
struct nested<
    TensorSlicingOp<StartIndices, Sizes, XprType>, 1,
...
class TensorSlicingOp
 ...
 public:
  ...
  typedef typename Eigen::internal::traits<TensorSlicingOp>::StorageKind
      StorageKind;
  typedef typename Eigen::internal::traits<TensorSlicingOp>::Index Index;

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorSlicingOp(
      const XprType& expr, const StartIndices& indices, const Sizes& sizes)
...
struct MemcpyTriggerForSlicing<Index, GpuDevice, BlockAccess> {
  ...
  EIGEN_DEVICE_FUNC bool operator()(Index, Index contiguous) const {
...
// Eval as rvalue
template <typename StartIndices, typename Sizes, typename ArgType,
          typename Device>
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>,
                       Device> {
  ...
  enum {
    ...
    BlockAccess =
        ...
        !internal::is_same<typename internal::remove_const<Scalar>::type,
                           bool>::value,
  ...
  typedef
      typename TensorEvaluator<const ArgType, Device>::TensorBlock TensorBlock;
  //===--------------------------------------------------------------------===//
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
      ...
      : m_impl(op.expression(), device),
        m_device(device),
        m_dimensions(op.sizes()),
        m_offsets(op.startIndices()) {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      // Don't initialize m_fastOutputStrides[0] since it won't ever be
      // accessed.
      m_outputStrides[0] = 1;
      for (int i = 1; i < NumDims; ++i) {
        m_outputStrides[i] = m_outputStrides[i - 1] * output_dims[i - 1];
        // NOTE: if tensor is degenerate, we send 1 to prevent TensorIntDivisor
        // constructor crash
        m_fastOutputStrides[i] = internal::TensorIntDivisor<Index>(
            degenerate ? 1 : m_outputStrides[i]);
      }
    } else {
      ...
      // Don't initialize m_fastOutputStrides[NumDims-1] since it won't ever be
      // accessed.
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(
    ...
    if (!NumTraits<typename internal::remove_const<Scalar>::type>::
      ...
      // Use memcpy if it's going to be faster than using the regular
      // evaluation.
      const MemcpyTriggerForSlicing<Index, Device, BlockAccess> trigger(
          m_device);
      if (trigger(internal::array_prod(dimensions()), contiguous_values)) {
        EvaluatorPointerType src = (EvaluatorPointerType)m_impl.data();
        for (Index i = 0; i < internal::array_prod(dimensions());
             i += contiguous_values) {
          Index offset = srcCoeff(i);
          m_device.memcpy((void*)(m_device.get(data + i)),
                          m_device.get(src + offset),
  ...
  packet(Index index) const {
    ...
    if (inputIndices[1] - inputIndices[0] == packetSize - 1) {
      PacketReturnType rslt =
          m_impl.template packet<Unaligned>(inputIndices[0]);
      return rslt;
    } else {
...
struct TensorEvaluator<TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
    : public TensorEvaluator<
          const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device> {
  typedef TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>,
                          Device>
      Base;
  typedef TensorSlicingOp<StartIndices, Sizes, ArgType> XprType;
  static const int NumDims = internal::array_size<Sizes>::value;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void writePacket(
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      for (int i = NumDims - 1; i > 0; --i) {
        ...
        inputIndices[0] +=
            (idx0 + this->m_offsets[i]) * this->m_inputStrides[i];
        inputIndices[1] +=
            (idx1 + this->m_offsets[i]) * this->m_inputStrides[i];
...
template <typename StartIndices, typename StopIndices, typename Strides,
          typename XprType>
struct traits<
    TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType> >
    : public traits<XprType> {
  typedef typename XprType::Scalar Scalar;
  typedef traits<XprType> XprTraits;
  typedef typename XprTraits::StorageKind StorageKind;
...
struct eval<
    TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType>,
    Eigen::Dense> {
  typedef const TensorStridingSlicingOp<StartIndices, StopIndices, Strides,
                                        XprType>
...
template <typename StartIndices, typename StopIndices, typename Strides,
          typename XprType>
struct nested<
    TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType>, 1,
    typename eval<TensorStridingSlicingOp<StartIndices, StopIndices, Strides,
                                          XprType> >::type> {
  typedef TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType>
      type;
};
...
class TensorStridingSlicingOp
    : public TensorBase<TensorStridingSlicingOp<StartIndices, StopIndices,
                                                Strides, XprType> > {
 public:
  typedef TensorBase<
      TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType> >
  ...
  typedef typename internal::traits<TensorStridingSlicingOp>::StorageKind
      StorageKind;
...
// Eval as rvalue
template <typename StartIndices, typename StopIndices, typename Strides,
          typename ArgType, typename Device>
struct TensorEvaluator<
    const TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>,
    Device> {
  typedef TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>
      XprType;
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType& op,
    ...
    // Handle degenerate intervals by gracefully clamping and allowing
    // m_dimensions to be zero
    ...
    typedef
        typename TensorEvaluator<ArgType, Device>::Dimensions InputDimensions;
 ...
 protected:
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index clamp(Index value,
                                                           Index min,
                                                           Index max) {
...
template <typename StartIndices, typename StopIndices, typename Strides,
          typename ArgType, typename Device>
struct TensorEvaluator<
    TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>,
    Device>
    : public TensorEvaluator<const TensorStridingSlicingOp<
                                 StartIndices, StopIndices, Strides, ArgType>,
                             Device> {
  typedef TensorEvaluator<const TensorStridingSlicingOp<
                              StartIndices, StopIndices, Strides, ArgType>,
                          Device>
      Base;
  typedef TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>
