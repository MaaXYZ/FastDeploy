...
static void test_full_reductions() {
  ...
  TensorMap<Tensor<Type, 2, DataLayout> > in_gpu(gpu_in_ptr, num_rows,
                                                 num_cols);
...
static void test_first_dim_reductions() {
  ...
  Eigen::TensorMap<Eigen::Tensor<Type, 3, DataLayout> > gpu_in(in_data, dim_x,
                                                               dim_y, dim_z);
  Eigen::TensorMap<Eigen::Tensor<Type, 2, DataLayout> > gpu_out(out_data, dim_y,
                                                                dim_z);
  ...
  dev.memcpyDeviceToHost(redux_gpu.data(), out_data,
                         gpu_out.size() * sizeof(Type));
  dev.synchronize();
...
static void test_last_dim_reductions() {
  ...
  Eigen::TensorMap<Eigen::Tensor<Type, 3, DataLayout> > gpu_in(in_data, dim_x,
                                                               dim_y, dim_z);
  Eigen::TensorMap<Eigen::Tensor<Type, 2, DataLayout> > gpu_out(out_data, dim_x,
                                                                dim_y);
