...
void test_cuda_complex_cwise_ops() {
  ...
  Eigen::TensorMap<Eigen::Tensor<std::complex<T>, 1, 0, int>, Eigen::Aligned>
      gpu_in1(d_in1, kNumItems);
  Eigen::TensorMap<Eigen::Tensor<std::complex<T>, 1, 0, int>, Eigen::Aligned>
      gpu_in2(d_in2, kNumItems);
  Eigen::TensorMap<Eigen::Tensor<std::complex<T>, 1, 0, int>, Eigen::Aligned>
      gpu_out(d_out, kNumItems);
  ...
  for (int op = Add; op < NbOps; op++) {
    ...
    assert(cudaMemcpyAsync(actual.data(), d_out, complex_bytes,
