...
struct GPUContext {
  GPUContext(const Eigen::TensorMap<Eigen::Tensor<float, 3> >& in1,
             Eigen::TensorMap<Eigen::Tensor<float, 3> >& in2,
    ...
    assert(gpuMemcpy(kernel_1d_, kernel_1d_val, 2 * sizeof(float),
                     gpuMemcpyHostToDevice) == gpuSuccess);

    assert(gpuMalloc((void**)(&kernel_2d_), 4 * sizeof(float)) == gpuSuccess);
    float kernel_2d_val[] = {3.14f, 2.7f, 0.2f, 7.0f};
    assert(gpuMemcpy(kernel_2d_, kernel_2d_val, 4 * sizeof(float),
                     gpuMemcpyHostToDevice) == gpuSuccess);

    assert(gpuMalloc((void**)(&kernel_3d_), 8 * sizeof(float)) == gpuSuccess);
    float kernel_3d_val[] = {3.14f, -1.0f, 2.7f, -0.3f,
                             0.2f,  -0.7f, 7.0f, -0.5f};
    assert(gpuMemcpy(kernel_3d_, kernel_3d_val, 8 * sizeof(float),
                     gpuMemcpyHostToDevice) == gpuSuccess);
  }
  ~GPUContext() {
...
void test_contextual_eval(Context* context) {
  context->out().device(context->device()) =
      context->in1() + context->in2() * 3.14f + context->in1().constant(2.718f);
}

template <typename Context>
void test_forced_contextual_eval(Context* context) {
  context->out().device(context->device()) =
      (context->in1() + context->in2()).eval() * 3.14f +
      context->in1().constant(2.718f);
}
...
void test_1d_convolution(Context* context) {
  ...
  context->out().slice(indices, sizes).device(context->device()) =
      context->in1().convolve(context->kernel1d(), dims);
}
...
void test_2d_convolution(Context* context) {
  ...
  context->out().slice(indices, sizes).device(context->device()) =
      context->in1().convolve(context->kernel2d(), dims);
}
...
void test_3d_convolution(Context* context) {
  ...
  context->out().slice(indices, sizes).device(context->device()) =
      context->in1().convolve(context->kernel3d(), dims);
}
...
void test_cpu() {
    ...
    for (int j = 0; j < 50; ++j) {
      for (int k = 0; k < 70; ++k) {
        VERIFY_IS_APPROX(out(i, j, k),
                         in1(i, j, k) + in2(i, j, k) * 3.14f + 2.718f);
      }
    }
  }
  ...
  for (int i = 0; i < 40; ++i) {
    for (int j = 0; j < 49; ++j) {
      for (int k = 0; k < 70; ++k) {
        VERIFY_IS_APPROX(out(i, j, k),
                         (in1(i, j, k) * 3.14f + in1(i, j + 1, k) * 2.7f));
      }
    }
  }
...
void test_gpu() {
  ...
  test_contextual_eval(&context);
  assert(gpuMemcpy(out.data(), d_out, out_bytes, gpuMemcpyDeviceToHost) ==
         gpuSuccess);
  for (int i = 0; i < 40; ++i) {
    for (int j = 0; j < 50; ++j) {
      for (int k = 0; k < 70; ++k) {
  ...
  assert(gpuMemcpyAsync(out.data(), d_out, out_bytes, gpuMemcpyDeviceToHost,
                        context.device().stream()) == gpuSuccess);
