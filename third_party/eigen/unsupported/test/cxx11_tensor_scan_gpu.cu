...
void test_gpu_cumsum(int m_size, int k_size, int n_size) {
  ...
  Eigen::TensorMap<Eigen::Tensor<float, 3, DataLayout> > gpu_t_input(
      d_t_input, Eigen::array<int, 3>(m_size, k_size, n_size));
  Eigen::TensorMap<Eigen::Tensor<float, 3, DataLayout> > gpu_t_result(
      d_t_result, Eigen::array<int, 3>(m_size, k_size, n_size));
