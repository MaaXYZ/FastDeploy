...
template <typename Kernel, typename Input, typename Output>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void run_on_gpu_meta_kernel(
    const Kernel ker, int n, const Input* in, Output* out) {
...
void run_on_gpu(const Kernel& ker, int n, const Input& in, Output& out) {
  ...
  gpuMemcpy(const_cast<typename Input::Scalar*>(in.data()), d_in, in_bytes,
...
template <typename Kernel, typename Input, typename Output>
void run_and_compare_to_gpu(const Kernel& ker, int n, const Input& in,
                            Output& out) {
  Input in_ref, in_gpu;
...
void ei_test_init_gpu() {
  ...
  std::cout << "  capability:                  " << deviceProp.major << "."
            << deviceProp.minor << "\n";
